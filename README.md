...
We implement: 
- some tests on a toy autoregressive model, a GARCH model, and fractional Brownian motion, where we estimate the Hurst index and evaluate both marginal and temporal dependency metrics. (see `test_AR.ipynb`, `test_GARCH` and `test_fBM`).
